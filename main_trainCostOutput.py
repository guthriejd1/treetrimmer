...
# Description: 
# Trains a neural network to match the optimal cost of a parametric mixed-integer quadratic program
...
matFile = r'C:\Users\guthrjd1\Documents\Graduate Studies\JHU AMS Deep Learning for Discrete Optimization\Project\Final Code\vehicleData_Cost.mat'
...
nbatch = 0
for alpha in [0.0005]:#[0.0001, 0.0005, 0.001, 0.005, 0.01, 0.05]:
    for n_hidden_units in [20]:#[1, 2, 4, 8, 16, 32, 64]:
        for lambda_l1 in [0]:
            for n_layers in [2]:
                ...
                model, history = train.trainNN_Cost(x_train_scaled, y_train_scaled, x_test_scaled, y_test_scaled, hyperparams)
                ...
                y_pred_train = standardScalerY.inverse_transform(y_pred_train_scaled)
                y_pred_test = standardScalerY.inverse_transform(y_pred_test_scaled)

                # save results
                result = {'y': y, 'y_test': y_test, 'y_train': y_train, 'y_pred_train': y_pred_train, 'y_pred_test': y_pred_test, 'y_pred': y_pred}
                result['y_train_scaled'] = y_train_scaled
                result['y_test_scaled'] = y_test_scaled
                result['y_pred_scaled'] = y_pred_scaled
                result['y_pred_train_scaled'] = y_pred_train_scaled
                result['y_pred_test_scaled'] = y_pred_test_scaled
                ...
                sio.savemat('result_Cost_' + str(nbatch), result)    
